            ...
            <div className="border-t border-gray-200">
                ...
                {data.map((data) => (
                    <div key={data.id} className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">{data.questions.question}</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{data.answers.answer}</dd>
                    </div>
                ))
                }
